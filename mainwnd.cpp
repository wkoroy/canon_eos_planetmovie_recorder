/***************************************************************************
 *   Copyright (C) 2008-2010 by –ß–µ—Ä–Ω–æ–≤ –ê.–ê.                                *
 *   valexlin@gmail.com                                                    *
 *                                                                         *
 ...
 : QWidget(0)
{
    ...
    tracking = new QCheckBox(this);
    tracking->setText("Tracking planet");
    ...
    focus_layout->addWidget(tracking, 0);

    wBbrightnplanet = new QSpinBox(this);
    wBbrightnplanet->setMinimum(1);
    wBbrightnplanet->setMaximum(255);
    wBbrightnplanet->setValue(20);
    wBbrightnplanet->setSingleStep(1);

    wBbrightnplanet->setToolTip(tr("Select brightness for planet"));
    focus_layout->addWidget(wBbrightnplanet, 0);



    wbTrackFreq = new QSpinBox(this);
    wbTrackFreq->setMinimum(2);
    wbTrackFreq->setMaximum(10000);
    wbTrackFreq->setValue(25);
    wbTrackFreq->setReadOnly(false);
    wbTrackFreq->setSingleStep(1);

    wbTrackFreq->setToolTip(tr("Select tracking frequensy (frames)"));
    focus_layout->addWidget(wbTrackFreq, 0);
    ...
    connect(HistBtn, SIGNAL(clicked()), this, SLOT(slotHistogram()));
     ...
     connect(tracking, SIGNAL(clicked()), this, SLOT(slottracking()));
     connect(wBbrightnplanet, SIGNAL(valueChanged( int )), this, SLOT(slotBrightPlanet()));
     connect(wbTrackFreq, SIGNAL(valueChanged( int )), this, SLOT(slotTrackFreq()));
    ...
    CurrSettings.trackfreq=5;
    CurrSettings.istrack=true;
    CurrSettings.delta=20;
	...
	// –æ—Ç–∫–ª—é—á–µ–Ω–æ –ø–æ –ø—Ä–æ—Å—å–±–µ —Ñ–æ—Ä—É–º—á–∞–Ω (forum.ixbt.com)
...
void GEOSRecWnd::keyPressEvent(QKeyEvent *event)
{
    int key=event->key();//event->key() - ˆÂÎÓ˜ËÒÎÂÌÌ˚È ÍÓ‰ ÍÎ‡‚Ë¯Ë
     QString str = QString(QChar(key));
   //  if (key>=Qt::Key_0 && key<=Qt::Key_9) { //÷ËÙÓ‚˚Â ÍÎ‡‚Ë¯Ë 0..9
  //    label1->setText(oldText+str);
   //   }
//     else if (key==Qt::Key_Backspace) { //BackSpace ÒÚË‡ÂÚ ÒËÏ‚ÓÎ
...
void GEOSRecWnd::loadSettings()
{
    ...
    CurrSettings.delta = settings.value(QString("delta"), 20).toInt();
    CurrSettings.trackfreq = settings.value(QString("trackfreq"), 25).toInt();
    CurrSettings.istrack = settings.value(QString("istrack"), false).toBool();
...
void GEOSRecWnd::saveSettings()
{
    ...
    settings.setValue(QString("delta"), QVariant(wBbrightnplanet->value()));
    settings.setValue(QString("trackfreq"), QVariant(wbTrackFreq->value()));
    settings.setValue(QString("istrack"), QVariant(tracking->isChecked()));
	...
	case CAMERA_EVENT_LV_STARTED:
		{
            ...
            CaptureWnd->settracking(CurrSettings.istrack);
            CaptureWnd->settrackfreq(CurrSettings.trackfreq);
            CaptureWnd->setdelta(CurrSettings.delta);

            wBbrightnplanet->setValue(CurrSettings.delta);
            wbTrackFreq->setValue(CurrSettings.trackfreq);
            tracking->setChecked(CurrSettings.istrack);
...
void GEOSRecWnd::slotMovelzoom()
{
    QPoint p;
    p.setX(wBbrightnplanet->value());
    p.setY(wbTrackFreq->value());

   // p.setX(40);
   // p.setY(40);

    setZoom(abs(5-zoomcurrent));
    CaptureWnd->movezoompos(p);
    setZoom(abs(5-zoomcurrent));
...
void GEOSRecWnd::slottracking()
{
    CaptureWnd->settracking(tracking->isChecked());
}
void GEOSRecWnd::slotBrightPlanet()
{
 CaptureWnd->setdelta( wBbrightnplanet->value());
 qDebug()<< wBbrightnplanet->value();
}


void  GEOSRecWnd::slotTrackFreq()
{
    CaptureWnd->settrackfreq( wbTrackFreq->value());
    qDebug()<< wbTrackFreq->value();
}
void GEOSRecWnd::slotMoverzoom()
{
    QPoint p;
    p.setX(-1*wBbrightnplanet->value());
    p.setY(-1*wbTrackFreq->value());
...
void  GEOSRecWnd::setZoom(int zoom)
{
    if (LiveThread && LiveThread->isInit())
    {
     zoomcurrent=zoom;
